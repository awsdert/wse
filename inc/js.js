...
// Get ID
function js () { var i = arguments[0]; if (doc.getElementById) { return doc.getElementById(i); } if (doc.all) { return doc.all[i]; } return null; }
// Create GET Object
js.get = function () { var r = false; if (!GET) { var g = {}, s = loc.search.slice(1); if (s !== "") { var l = s.split("&"), t; for (var i = 0;i < l.length;i++) { t = l[i].split("="); if (!t[1] || /^TRUE$/i.test(t[1])) { g[t[0]] = true; } else if (/^FALSE$/i.test(t[1])) { g[t[0]] = false; } else if (/^\-?(\d|\,)+(\.(\d|\,)+)?$/.test(t[1])) { g[t[0]] = (+t[1].replace(/\,/g, "")); } else { g[t[0]] = decodeURI(t[1]); } } GET = g; r = true; } } else { r = true; } return r; };
// Get Tags
js.tags = function () {
	var a = arguments; var e = a[0], n = a[1], v = a[3], r = []; a = a[2];
	if (e && e.nodeName && js.def.txt(n) && n !== "") {
...
// Get / Set Inner HTML
js.html = function () { var a = arguments; var e = a[0], c = a[1], r = ""; if (e && e.innerHTML) { if (js.def.txt(c)) e.innerHTML = c; r = e.innerHTML; } return r; };
// Get / Set Attribute
js.attr = function () {
	var a = arguments; var e = a[0], v = a[2], r = null, a = a[1].toLowerCase();
	if (e && e.nodeName && js.def.txt(a) && a !== "") {
		if (js.def.txt(v)) {
...
*/
js.def = function () { if (arguments[0]) return true; return false; };
js.def.txt = function () { if (typeof arguments[0] === "string") return true; return false; };
js.def.num = function () { if (typeof arguments[0] === "number") return true; return false; };
js.def.bol = function () { if (typeof arguments[0] === "boolean") return true; return false; };
js.def.fun = function () { if (typeof arguments[0] === "function") return true; return false; };
js.def.obj = function () {
	var a = arguments;
	var b = false;
...
js.ajax = function () {
	...
	if (js.def.txt(o)) { x = o; o = { u : x }; x = null; }
	var u = o.u; x = js.ajax.xhr(); if (!d) { d = null; }
	if (!js.def.fun(f)) { f = function () {}; }
	if (js.def.obj(o) && x && js.def.txt(u) && u !== "") {
		a = o.a; var m = o.m, s = o.s;
		if (!js.def.bol(a)) { a = true; }
		if (js.def.txt(m)) {
			m = m.toUpperCase();
...
// Get Web Document
js.ajax.dom = function () {
	var a = arguments; var d = [a[2], a[1]];
	js.ajax(a[0], function () {
...
// Get Feed
js.ajax.rss = function () {
	var a = arguments; var d = [a[2], a[1]];
	js.ajax.dom(a[0], function () {
		var a = arguments;
...
js.parse = {
	dom : function () {
		...
		if (js.def.txt(n) && /^([0-9]|\,)+(\.([0-9]|\,)+)?$/.test(n))
			{ x = (+a[0].replace(",", "")); }
		return x;
	}, json : function () {
	...
	tab : function () {
		...
		if (e && e.nodeName && e.href) {
			var u = e.href.split("#")[1].split("?"), l, t, i,
				d = js.ui.tab.data, f = js.ui.tab.ajax;
			...
			for (i = 0;i < t.length;i++) {
				l = u + t[i] + ".txt";
				if (!d) { js.ajax(l, f, [t[i], e, a[1]]); } else {
					 if (t[i] !== d[i]) {
					 	js.ajax(l, f, [t[i], e, a[1]]);
					 }
				}
			} js.ui.tab.data = t;
			return false;
		...
		if (e.id && e.value && /^(\d|\,)+(\.(\d|\,)+)?$/.test(e.value)) {
			var t = js(e.id + "-txt"), b = js(e.id + "-bar"), v = e.value(/\,/g, "") + "%";
			if (t) { js.html(t, v); }
			if (b) { js.css(b, "width", v); }
		}
	}
};
// Tab UI Dependancies
js.ui.tab.data = null;
js.ui.tab.ajax = function () {
	var a = arguments; var e = a[1][1];
	if (!a[0]) { window.location.href = e.href; }
	else {
		var p = e.parentNode; var b = js(p.id + "-box");
		if (/^FUNCTION/i.test(a[0])) { if (js.def.fun(a[2])) { a[2](b, a[0].slice(8)); } }
		// Comment out if your text files are secure
		else if (/\<script/i.test(a[0])) { js.html(b, "Error: SCRIPT tags are not allowed via js.ui.tab()."); }
		else { js.html(b, a[0]); }
		a = js.tags(p, "a");
		for (var i = 0;i < a.length;i++) { a[i].className = ""; }
		e.className = "tabx";
		e = js(e.href.split("#")[1].split("?")[0]);
		if (e.parentNode.className === "tabl") { js.ui.tab(e); }
	}
};